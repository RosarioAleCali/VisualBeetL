...
char* PktDef::GenPacket()
{
	...
	RawBuffer = new char[this->comPkt.header.length];
	memset(RawBuffer, 0x00, this->comPkt.header.length);
	memcpy(RawBuffer, &comPkt.header, HEADERSIZE);

	if (this->comPkt.header.length <= (HEADERSIZE + 1)) {
		...
		RawBuffer[this->comPkt.header.length - 1] = comPkt.CRC;
	}
	
	return RawBuffer;
}

bool PktDef::GetDrive()
{
	return comPkt.header.Drive;
}
